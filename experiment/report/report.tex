...
结果表明，本地 8B 解释生成稳定可行：800 条释义全部成功，平均耗时 \QwenGenerationMeanSeconds\,s/条，端到端耗时 \QwenGenerationWallSeconds\,s，峰值显存 \QwenGenerationPeakGiB\,GiB。字符 TF--IDF 分类器在 400/400 子集上的 Macro-F1 从仅弹幕的 \SmallContentMacroF\% 提升到本地 Qwen 释义的 \SmallQwenMacroF\%，但配对行 bootstrap 的增量为 \SmallQwenDeltaPP\ 个百分点，95\% CI 为 [\SmallQwenDeltaLowPP, \SmallQwenDeltaHighPP]，仍包含 0；直接拼接原始前 5 条则下降到 \SmallRawMacroF\%。在完整发布划分上，仓库官方释义将 Macro-F1 从 \FullContentMacroF\% 提高到 \FullOfficialMacroF\%，配对增量为 \FullOfficialDeltaPP\ 个百分点（95\% CI [\FullOfficialDeltaLowPP, \FullOfficialDeltaHighPP]）。Chinese-RoBERTa 在 2,000/2,000 设置中由 \RobertaMainContentMean$\pm$\RobertaMainContentSD\% 提高到 \RobertaMainOfficialMean$\pm$\RobertaMainOfficialSD\%，但在 400/400 设置中本地 Qwen 释义反而下降 \RobertaSmallQwenDeltaPP\ 个百分点。追加实验采用对象--行为紧凑表示和训练集内部验证的晚期融合，在 1,000 条训练设置下将 RoBERTa Macro-F1 提高到 \ImproveRobertaLargeLateFusion\%，高于纯当前弹幕的 \ImproveRobertaLargeContent\%。在完整 9,069 条测试集、每类 50/60/70 条训练和验证的同口径实验中，三次独立运行的内容均值为 \PaperMatchedContentMeanFifty\%、\PaperMatchedContentMeanSixty\% 和 \PaperMatchedContentMeanSeventy\%，均低于论文 Table 5 的均值；预先定义的三种子概率集成点分别为 \PaperMatchedContentEnsembleFifty\%、\PaperMatchedContentEnsembleSixty\% 和 \PaperMatchedContentEnsembleSeventy\%，其中 70-shot 相对论文点估计高 \PaperMatchedContentEnsembleDeltaSeventy 个百分点。该集成优势是描述性工程结果，不是把单个集成点当成论文式均值±SD，也不是 SPT-RII 完整复现。最后，在 1,000/400 子集上用统一 \texttt{think=False} 配置和验证集选择 Qwen 不确定性门控，测试 Macro-F1 达到 \GateMacroF\%，比 TF--IDF 基线提高 \GateDeltaPP 个百分点，但 CI 为 [\GateCILowPP, \GateCIHighPP]，仍属探索性结果。因此，单卡运行本地 8B 解释器是现实可行的，但解释增强的收益依赖解释压缩方式、融合策略、下游分类器以及训练规模，不能概括为无条件增益。
...
\begin{enumerate}[leftmargin=2em,itemsep=0.25em]
  ...
  \item 与论文 Table 5 同口径的三次独立运行均值在 70-shot 为 \PaperMatchedContentMeanSeventy\%（相对 -1.23 pp）；预先定义的三种子概率集成点为 \PaperMatchedContentEnsembleSeventy\%（相对 \PaperMatchedContentEnsembleDeltaSeventy pp）。后者必须与“未实现 SPT-RII 全部模块”的边界一起解读。
  \item 验证集选择的不确定性门控在 400 条测试上达到 \GateMacroF\%，相对 TF--IDF 基线提升 \GateDeltaPP 个百分点，但 95\% CI [\GateCILowPP,\,\GateCIHighPP] 跨 0，只能视为下一轮实验的候选方向。
\end{enumerate}
\end{keybox}
...
\begin{riskbox}[复现边界]
本实验没有实现 soft prompt、BiLSTM prompt 交互、动态 verbalizer 或 POS/语义锚点更新；后续新增了 50/60/70-shot 的测试范围匹配实验，但下游仍是替代性的 RoBERTa/TF--IDF 方案，而非 SPT-RII 本身。它回答的是：在 4060 8 GB 上，是否能运行本地 8B 解释器，以及解释作为输入增强是否有可测收益。该边界在结果解释中始终保留。
\end{riskbox}
...
\label{tab:roberta}
\resizebox{\textwidth}{!}{\input{table_roberta.tex}}
\end{table}
...
\subsection{补充实验：与原论文 Table 5 的同口径比较}

为检验本地方案是否在原论文的低样本设定下具有竞争力，新增一组不使用小规模测试子集的正式对比。实验固定使用发布的 RecDY 完整测试集（$n=9{,}069$），分别按每类 50、60、70 条抽取训练集，并另抽取同样数量且与训练不重叠的验证集；训练和验证抽样使用相同的三种子（100、101、102），测试集始终不变。论文 Table 5 的统计对象是三次独立运行的均值±SD，因此本报告首先报告本地三次独立运行均值±样本 SD，再把预先定义的三种子概率平均作为单独的工程集成点。所有阈值和融合权重只由验证集选择。该协议与论文 Table 5 的 shot 计数和测试范围一致，但论文的重复采样和初始化细节未完全公开；下游模型也仍是本地 RoBERTa，而不是论文的 soft prompt、BiLSTM prompt 交互和 dynamic verbalizer。

表~\ref{tab:paper-comparison} 和图~\ref{fig:paper-comparison} 给出了逐档结果。原论文 SPT-RII 的报告值为 50-shot 的 $77.84\pm0.32\%$、60-shot 的 $78.92\pm0.56\%$ 和 70-shot 的 $79.23\pm0.31\%$。本地三次独立运行的内容均值分别为 \PaperMatchedContentMeanFifty\%、\PaperMatchedContentMeanSixty\% 和 \PaperMatchedContentMeanSeventy\%，相对论文点差为 $\PaperMatchedContentMeanDeltaFifty$、$\PaperMatchedContentMeanDeltaSixty$ 和 $\PaperMatchedContentMeanDeltaSeventy$ 个百分点。独立运行均值在三档都低于论文报告点；这才是与论文均值±SD最直接的比较。预先定义的三种子概率集成点分别为 \PaperMatchedContentEnsembleFifty\%、\PaperMatchedContentEnsembleSixty\% 和 \PaperMatchedContentEnsembleSeventy\%，70-shot 的单点相对论文高 $\PaperMatchedContentEnsembleDeltaSeventy$ pp。集成点不是三次独立运行均值，不能用其替代论文式重复实验统计。

在同一验证集上再加入字符 TF--IDF 概率作为第二个独立视角，三次独立运行的 clean-fusion 均值在 50/60/70-shot 分别为 \PaperMatchedFusionMeanFifty\%、\PaperMatchedFusionMeanSixty\% 和 \PaperMatchedFusionMeanSeventy\%，仍低于论文点估计；对应的三种子融合集成点在 70-shot 为 \PaperMatchedFusionEnsembleSeventy\%（相对论文 \PaperMatchedFusionEnsembleDeltaSeventy pp），但 50/60-shot 并未提高。因此正文把独立运行均值作为论文同口径主比较，把概率集成和 TF--IDF 融合标为次级工程增强，而不是宣称所有 shot 档位都受益。论文 Table 6 另列 SPT-RII 50-shot F1=78.32，与 Table 5 的 77.84 不一致；本报告以表头明确的 Table 5 为主，并保留该内部不一致记录。

官方仓库释义与本地内容融合的 50/60/70-shot 集成结果为 \PaperMatchedOracleFifty\%、\PaperMatchedOracleSixty\% 和 \PaperMatchedOracleSeventy\%，仅作为“解释已接近标签语义时”的上限参考。由于这些释义来自仓库并且语义上含有明显的任务先验，官方分支不进入干净主结果，也不用于证明本地 Qwen 或 RoBERTa 的泛化能力。

\begin{table}[htbp]
\centering
\small
\caption{与原论文 RecDY Table 5 的同口径描述性比较。Macro-F1 为百分比；独立运行行报告三次运行的样本均值±SD，三种子概率集成行是单个预先定义的集成点，因此 SD 标为 ``--''。论文 SD 为论文报告的重复标准差。}
\label{tab:paper-comparison}
\resizebox{\textwidth}{!}{\input{table_paper_comparison.tex}}
\end{table}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{fig6_paper_comparison.pdf}
  \caption{与原论文 Table 5 的全量 RecDY 测试比较。a，论文均值、本地独立运行均值及单独标出的三种子概率集成点；b，论文与本地独立运行的 SD、集成点和官方释义 oracle-only 参考。70-shot 集成点超过论文报告点，但独立运行均值没有超过；本地实现不含 SPT-RII 的 soft prompt 和动态 verbalizer，图中结论仅为同测试协议下的描述性比较。}
  \label{fig:paper-comparison}
\end{figure}
...
\resizebox{\textwidth}{!}{\input{table_improvement.tex}}
\end{table}
...
\input{table_roberta_improvement.tex}
\end{table}
...
\subsection{选择性 Qwen 二次意见：不确定性门控}

前述晚期融合把 Qwen 释义作为所有样本的输入；另一个更节省推理的假设是，只在监督分类器不确定时请求 Qwen 直接给出第二意见。为避免测试集调参，固定 1,000 条训练集再划出 20\%（200 条）验证集，先用训练子集拟合字符 TF--IDF，计算当前弹幕的正类概率 $p$，再在验证集网格搜索不确定区间 $|p-0.5|\leq m$。落入区间的样本才用 Qwen3-8B 直接分类结果替换原预测；$m$ 和替换规则完全由验证集 Macro-F1 选择，测试集只执行一次。验证和测试均重新使用同一推理配置：\texttt{think=False}、temperature 0、\texttt{num\_predict=8}、\texttt{num\_ctx=512}；旧版直接分类缓存不参与主结果。

验证集选择 $m=\GateMargin$，即 $p\in[\GateLower,\GateUpper]$，覆盖 \GateValidationReplacementPct\% 的验证行。固定的 400 条测试集上，基线 TF--IDF 为 \GateBaseMacroF\%，Qwen 零样本直接分类仅为 \GateQwenMacroF\%，而选择性门控达到 \GateMacroF\%，相对基线提高 \GateDeltaPP 个百分点；实际只替换 \GateReplacementPct\% 的测试行。5,000 次行级配对 bootstrap 的 95\% CI 为 [\GateCILowPP,\,\GateCIHighPP] 个百分点，跨过 0，因此该结果应称为探索性趋势，而不是已证实的显著提升。

这项结果给出了比“全量拼接解释”更具体的工程方向：Qwen 不必承担全部样本的推理成本，监督模型的置信度可以作为路由信号。验证集 200 条和测试集 400 条的新配置 Qwen 调用记录推理时间合计分别约 \GateValidationQwenSeconds\,s 与 \GateTestQwenSeconds\,s，600 条均有逐行 JSONL 检查点。部署时只对边界样本调用模型，仍需把路由收益与延迟预算一起评估。但当前验证集只有 200 行，且门控只在 400 行、12 个直播间上测试；此外，验证概率来自 800 条拟合模型，测试概率来自全部 1,000 条重训模型，固定区间可能受到概率校准漂移影响。下一轮应扩大测试集、按直播间聚类 bootstrap，并比较 RoBERTa 置信度与 TF--IDF 置信度的门控稳定性。

\begin{table}[H]
\centering
\small
\caption{验证集选择的不确定性门控结果（Macro-F1，\%）。}
\label{tab:gate}
\input{table_gate.tex}
\end{table}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\textwidth]{fig7_uncertainty_gate.pdf}
  \caption{选择性 Qwen 二次意见。a，固定 400 条测试集的 TF--IDF 基线、Qwen 直接分类和门控结果；b，验证集选择的区间迁移到测试集后的变化，浅色带为测试集配对 bootstrap 95\% CI。门控提升为探索性点估计，CI 跨过 0。}
  \label{fig:gate}
\end{figure}
...
\subsection{门控比全量调用更接近可部署策略}

不确定性门控把 Qwen 从“每条样本都调用的解释器”变成“只处理边界样本的第二意见”。在固定 400 条测试上，它将 TF--IDF 的 Macro-F1 从 \GateBaseMacroF\% 提高到 \GateMacroF\%，只替换 \GateReplacementPct\% 的行；Qwen 直接分类本身只有 \GateQwenMacroF\%。这说明增益主要来自路由，而不是把零样本大模型直接当作分类器。由于 CI [\GateCILowPP,\,\GateCIHighPP] 跨 0，且验证集门控区间在小样本上可能过拟合，当前证据只足以支持“值得扩大验证”的工程假设。
...
原论文的 SPT-RII 还包含 soft prompt 和动态词表，其 RecDY 50--70 shot 主表 F1 为 77.84--79.23。新增的 full-test 对比已经匹配了每类训练/验证 shot 和测试行数：本地三次独立运行均值在 70-shot 为 \PaperMatchedContentMeanSeventy\%，相对论文为 \PaperMatchedContentMeanDeltaSeventy pp；预先定义的三种子概率集成点为 \PaperMatchedContentEnsembleSeventy\%，相对论文为 \PaperMatchedContentEnsembleDeltaSeventy pp。这个结果支持“概率集成是一个值得继续评估的工程增强”，但不能把集成点当作论文式均值，也不能归因于 SPT-RII 的 soft prompt 或 dynamic verbalizer，更不能替代原论文的模块级消融。当前结果仍支持原论文一个较窄的思想：短弹幕可能受益于语境解释；同时补充了一个重要边界，即在低数据和高容量下游模型中，解释也可能成为噪声源。
...
\begin{enumerate}[leftmargin=2em,label=\arabic*.]
  ...
  \item \textbf{门控样本量、校准迁移与统计不确定性。} Qwen 二次意见只在 200 条验证和 400 条测试上评估，门控 CI 跨 0；区间由 800 条拟合模型的验证概率选出，再迁移到 1,000 条重训模型的测试概率，可能存在校准漂移。当前不能证明增益可迁移到 full-test 或未见直播间。
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{扩大选择性路由。} 在 full RecDY 或按直播间留出协议上复验不确定性门控，比较 TF--IDF、RoBERTa 和校准概率，并记录每条 Qwen 调用的延迟与成本。
\end{enumerate}

\section{结论}

本研究在 RTX 4060 Laptop GPU 8 GB 上完成了与 BC4RII 相关的解释增强缩小实验、官方仓库审计、full-test shot 匹配对比和完整复现性记录。Qwen3-8B Q4 能以约 \QwenGenerationMeanSeconds\,s/条稳定生成一句话释义，峰值整卡显存约 \QwenGenerationPeakGiB\,GiB，证明本地 8B 解释器具有现实可操作性。实验最稳健的发现不是“解释总能提高分类”，而是原始上下文直接拼接会严重干扰分类，语义压缩能够避免这种退化；至于是否超过仅当前弹幕，则取决于解释源、表示方式、融合策略、训练规模和下游模型。改进后的 TF--IDF 在 1,000 条设置达到 \ImproveTFIDFLargeStructuredLate\%，RoBERTa 经紧凑表示和验证集晚期融合达到 \ImproveRobertaLargeLateFusion\%，但 400 条设置仍只有恢复性结果。在完整 9,069 条测试集上，本地内容独立运行均值在 70-shot 为 \PaperMatchedContentMeanSeventy\%，没有超过论文 Table 5；三种子概率集成点达到 \PaperMatchedContentEnsembleSeventy\%，相对论文点估计为 \PaperMatchedContentEnsembleDeltaSeventy pp。在 1,000/400 子集上，验证集选择的 Qwen 不确定性门控达到 \GateMacroF\%，但其 CI [\GateCILowPP,\,\GateCIHighPP] 跨 0。上述优势属于概率集成和探索性路由策略的描述性结果，不是独立重复均值优势，也不是 SPT-RII 完整复现。因而，下一步研究应把重点从“换更大的模型”转向严格划分、解释忠实性、选择性路由、聚类级统计和更完整的样本量曲线。
...
\begin{lstlisting}[language={},caption={与原论文 Table 5 的 full-test shot 匹配实验}]
$env:HF_HOME='D:\hf'
...
\begin{lstlisting}[language={},caption={验证集不确定性门控（Qwen 二次意见）}]
...
\path{experiment/results/} & 每个分类条件的 JSON、逐行预测、配对 bootstrap、环境和数据审计；\texttt{improvement/} 含 OOF 选择和门控记录；\texttt{paper\_comparison/} 含 full-test shot 匹配结果 \\
\path{experiment/figures/} & 七张主图的 SVG/PDF/PNG/TIFF 与 source-data CSV；图6为原论文 Table 5 对比，图7为不确定性门控 \\
\path{experiment/src/run\_paper\_matched.py} & 每类 50/60/70 条训练与验证、固定 9,069 条测试的三种子运行器 \\
\path{experiment/src/run\_uncertainty\_gate.py} & 只用训练内部验证集选择 Qwen 二次意见的不确定性门控 \\
\path{experiment/src/make\_gate\_assets.py} & 门控结果的表格、图7、source data 和 QA 记录 \\
\path{experiment/paper\_reader/} & 论文全文提取、双语块锚点、Table 5 裁剪图和翻译审计说明（draft mode） \\
...
\path{experiment/report/report.pdf} & 编译后的最终报告（含改进章节、full-test 对比图6 和门控图7） \\
\bottomrule
\end{longtable}
...
full-test shot 匹配下 70-shot 内容集成高于论文点估计 & 9,069 条固定测试；三种子概率集成：81.65\% vs 79.23\%（+2.42 pp） & 本地替代模型，不含 soft prompt/dynamic verbalizer \\
验证集不确定性门控带来探索性增益 & 1,000/400：82.29\% 基线 $\to$ 83.71\%，+1.42 pp；CI [-2.61,+5.37] pp & 仅 200 条验证、400 条测试，CI 跨 0 \\
\bottomrule
\end{longtable}
